...
{	 
	...
	TIM5_ICInitStructure.TIM_Channel = TIM_Channel_1; //CC1S=01 	选择输入端 IC1映射到TI1上
  	TIM5_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Falling;	//下将沿捕获
...
u8  TIM5CH1_CAPTURE_STA=0;	//输入捕获状态	8位0x00~0x80	    				
u16	TIM5CH1_CAPTURE_VAL;	//输入捕获值,定时器溢出的次数
...
{	 
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_6;  //PA1清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PA1 输入  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB,GPIO_Pin_6);						 //PA1 下拉
	...
	//初始化TIM2输入捕获参数
	TIM4_ICInitStructure.TIM_Channel = TIM_Channel_1 ; //CC1S=10 	选择输入端 IC1映射到TI2上
  	TIM4_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Falling;	//下将沿捕获
...
u8  TIM4CH1_CAPTURE_STA=0;	//输入捕获状态	8位0x00~0x80	    				
u16	TIM4CH1_CAPTURE_VAL;	//输入捕获值,定时器溢出的次数
